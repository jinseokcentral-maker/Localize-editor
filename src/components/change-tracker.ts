...
export class ChangeTracker {
  ...
  initializeOriginalData(
    ...
    if (this.config.enableValidation) {
      ...
      for (const lang of languages) {
        const langEffect = validateWithEffect(
          LangSchema,
          lang,
      ...
      for (const t of translations) {
        const idEffect = validateWithEffect(
          RowIdSchema,
          t.id,
          `Invalid row ID: ${t.id}`,
        );
  ...
  getOriginalValueEffect(
    ...
    ChangeTrackerError | import("@/types/errors").ValidationError
  > {
  ...
  trackChange(
    ...
    // 검증 활성화 시 Effect 사용
    const effect = this.trackChangeEffect(
      rowId,
      field,
      lang,
      oldValue,
